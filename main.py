...
def _load_config():
    ...
    return {
        ...
        "cpu_alert_webhook": os.getenv(
            "CPU_ALERT_WEBHOOK", cfg.get("cpu_alert_webhook", "")
        ),
    }
...
class CPUWatchdog:
    ...
    async def _send_webhook_alert(self, total_pct: float, top3: list):
        ...
        embed = {
            ...
            "color": 0xE67E22,
            "footer": {
                "text": f"Host Guardian • {datetime.utcnow().isoformat(timespec='seconds')}Z"
            },
        }
        try:
    ...
    async def run(self):
        while True:
            ...
            if total >= self.threshold:
                ...
                if (
                    self._over_counter >= 2
                    and (time.time() - self._last_alert_ts) > self.cooldown
                ):
...
class Bot:
    ...
    def _register_commands(self):
        ...
        async def status(interaction: discord.Interaction):
            await interaction.response.send_message(
                embed=self._build_status_embed(), ephemeral=True
            )
        ...
        async def topcpu(interaction: discord.Interaction):
            ...
            self._last_topcpu_by_user[interaction.user.id] = {
                "at": time.time(),
                "rows": rows,
            }
        ...
        async def terminate(
            interaction: discord.Interaction, index: app_commands.Range[int, 1, 10]
        ):
            await interaction.response.defer(ephemeral=True, thinking=True)
    ...
    def _build_status_embed(self) -> discord.Embed:
        ...
        lines = [format_proc_line(i + 1, r) for i, r in enumerate(top3)] or [
            "_sampling…_"
        ]
